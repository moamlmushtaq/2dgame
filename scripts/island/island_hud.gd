...
func _draw() -> void:
	...
	Paint.text(self, Vector2(110, 34), "قطع الخريطة: %d" % Game.map_pieces, 18, Color.WHITE, true, 4, shadow)
	...
	if island.won and island.win_time > 0.8:
		...
		Paint.text(self, card.position + Vector2(300, 125), "حصلتم على قطعة الخريطة رقم %d" % Game.map_pieces, 24, ink)
		Paint.text(self, card.position + Vector2(300, 170), "البلورات: %d/%d" % [island.gems_found, island.gems_total], 22, Color(0.2, 0.6, 0.75, a))
		if island.win_time > 1.8:
			var blink := (0.6 + 0.4 * sin(_t * 4.0)) * a
			Paint.text(self, card.position + Vector2(300, 250), "اضغطوا زر التفاعل للإبحار إلى الجزيرة التالية", 20, Color(ink, blink))
			Paint.text(self, card.position + Vector2(300, 290), "Esc للعودة إلى القائمة", 15, Color(ink, 0.6 * a), false)
